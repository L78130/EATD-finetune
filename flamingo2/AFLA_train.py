...
if __name__ == "__main__":
    ...
    # get train dataset
    train_root = "/datasets/EATD-Corpus/flamingo2"
    ...
    train_dataset = data_load._load_split(train_root, trainFOLDER_PATTERN)
    print(f"Train dataset size: {len(train_dataset)} samples")

    # get test dataset
    test_root = "/datasets/EATD-Corpus/flamingo2"
    test_label_root = "\datasets\EATD-Corpus\EATD-Corpus"
    data_load.copy_label_files(test_label_root, test_root, testFOLDER_PATTERN)
    test_dataset = data_load._load_split(test_root, testFOLDER_PATTERN)
    ...
    # define model
    model = AFLA_linear.classifier(2688, 2).to(device)
